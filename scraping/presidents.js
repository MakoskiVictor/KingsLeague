...
const presidents = await Promise.all(
  RAW_PRESIDENTS.map(async presidentInfo => {
    ...
    console.log(`> Fetching img for president: ${name}`)
    // fetch de la img y guardarlo en el sistema
...
console.log('All presidents done!')
await writeFile(`${DB_PATH}/presidents.json`, JSON.stringify(presidents, null, 2), 'utf-8')

/* https://kingsleague.pro//wp-json/wp/v2/presidents?per_page=20 */
